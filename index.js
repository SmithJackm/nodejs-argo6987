...
// 2. 生成三协议 Base64 订阅内容
function getSubContent(argoDomain) {
  const vless = `vless://${UUID}@cdns.doon.eu.org:443?encryption=none&security=tls&sni=${argoDomain}&type=xhttp&mode=packet-up&host=${argoDomain}&path=%2Fvless#Railway_VLESS_2026`;
  
  const vmessJson = { v: '2', ps: 'Railway_VMess_2026', add: 'cdns.doon.eu.org', port: '443', id: UUID, aid: '0', scy: 'none', net: 'xhttp', type: 'none', host: argoDomain, path: '/vmess', tls: 'tls', sni: argoDomain, mode: 'packet-up'};
  const vmess = `vmess://${Buffer.from(JSON.stringify(vmessJson)).toString('base64')}`;
  
  const trojan = `trojan://${UUID}@cdns.doon.eu.org:443?security=tls&sni=${argoDomain}&type=xhttp&mode=packet-up&host=${argoDomain}&path=%2Ftrojan#Railway_Trojan_2026`;
...
async function bootstrap() {
  ...
  try {
    ...
    await axios({ method: 'get', url: `${baseUrl}/web`, responseType: 'stream' }).then(res => res.data.pipe(fs.createWriteStream(webPath)));
    await axios({ method: 'get', url: `${baseUrl}/bot`, responseType: 'stream' }).then(res => res.data.pipe(fs.createWriteStream(botPath)));
...
app.get("/", (req, res) => res.send("2026 XHTTP Active"));
app.get("/sub", (req, res) => res.send(getSubContent(ARGO_DOMAIN)));
